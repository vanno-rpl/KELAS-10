...
const educationJourney = [
  {
    ...
    title: 'Sekolah Dasar',
    period: 'Fondasi awal',
    text: 'Membangun kebiasaan belajar, disiplin, dan rasa ingin tahu.',
  },
  {
    level: 'SMP',
    title: 'Sekolah Menengah Pertama',
    period: 'Masa eksplorasi',
    text: 'Mulai mengenal minat, kerja kelompok, dan dasar teknologi.',
  },
  {
    level: 'SMK',
    title: 'Sekolah Menengah Kejuruan',
    period: 'Fokus keahlian',
    text: 'Mengasah keterampilan praktis, project, dan kesiapan masuk dunia kerja.',
  },
]
    ...
    <>
      <section className="hero-section">
        <div className="hero-copy">
          <p className="eyebrow">React + Laravel Portfolio</p>
          <h1>{data.profile.name}</h1>
          <p className="hero-headline">{data.profile.headline}</p>
          <p className="hero-text">{data.profile.bio}</p>
          <div className="hero-actions">
            <a className="button primary" href="#hobi">
              Explore Hobi <ArrowRight size={18} />
            </a>
      ...
      <section className="section-grid" id="hobi">
        <div className="section-heading">
          <Gamepad2 size={22} />
          <h2>Hobi</h2>
        </div>
        <HobbyGrid hobbies={data.hobbies} />
      </section>

      <section className="section-grid">
        <div className="section-heading">
          <GraduationCap size={22} />
          <h2>Perjalanan Studi</h2>
        </div>
        <div className="education-list">
          {educationJourney.map((item) => (
            <article className="education-item" key={item.level}>
...
function LoginPage() {
  ...
  const [form, setForm] = useState({ email: 'admin@vano.test', password: 'password' })
  const [error, setError] = useState('')
  ...
  return (
    <section className="login-page">
      <form className="login-panel" onSubmit={submit}>
        <UserRound size={32} />
        <h1>Login</h1>
        <p>Kelola konten portfolio Vano dari dashboard.</p>
        <Field label="Email" type="email" value={form.email} onChange={(value) => setForm({ ...form, email: value })} />
...
function AdminLayout() {
  ...
  const links = [
    ...
    ['Hobi', '/admin/hobbies', Gamepad2],
    ['Projects', '/admin/projects', BriefcaseBusiness],
    ['Experience', '/admin/experiences', PenLine],
  ...
  return (
    <div className="admin-shell">
      <aside className="admin-sidebar">
        <Link className="brand admin-brand" to="/admin"><span>V</span> Admin</Link>
...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([projects, skills, hobbies, experiences, messages]) => {
      setSummary({
  ...
  return (
    <section className="admin-section">
      ...
      <div className="stats-grid">
        ...
        <Stat label="Hobi" value={summary.hobbies} />
        <Stat label="Experiences" value={summary.experiences} />
...
function HobbiesAdmin() {
  return (
    <CrudAdmin
      title="Hobi"
      endpoint="/admin/hobbies"
      emptyItem={emptyHobby}
      normalize={(item) => ({ ...emptyHobby, ...item })}
      fields={[
        ['name', 'Name'],
      ...
      renderItem={(item) => `${item.name} · ${item.icon || 'Sparkles'}`}
    />
  )
}
...
function ProjectsAdmin() {
  return (
    <CrudAdmin
      ...
      normalize={(item) => ({ ...emptyProject, ...item, tech_stack: (item.tech_stack || []).join(', ') })}
      serialize={(item) => ({ ...item, tech_stack: splitList(item.tech_stack), is_featured: Boolean(item.is_featured) })}
      fields={[
        ['title', 'Title'],
        ['slug', 'Slug'],
        ['summary', 'Summary'],
      ...
      checkbox={['is_featured', 'Featured']}
      renderItem={(item) => `${item.title} · ${item.slug}`}
    />
  )
}
...
function CrudAdmin({ title, endpoint, emptyItem, fields, checkbox, normalize, serialize, renderItem }) {
  ...
  return (
    <section className="admin-section">
      <AdminTitle title={title} text={`Manage ${title.toLowerCase()} shown on the public website.`} />
...
function HobbyGrid({ hobbies }) {
  ...
  return (
    <div className="hobby-grid">
      {hobbies.map((hobby) => {
        const Icon = icons[hobby.icon] || Sparkles
...
function ImageUploadField({ label, value, onChange }) {
  ...
  function upload(event) {
    ...
    setUploading(true)
    setError('')
    api.post('/admin/uploads', data, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
      .then((response) => onChange(response.data.url))
      .catch((requestError) => setError(getErrorMessage(requestError)))
  ...
  return (
    <div className="upload-field">
      ...
      {value ? <img src={value} alt={`${label} preview`} /> : null}
      {error ? <p className="alert error">{error}</p> : null}
    </div>
  )
}
